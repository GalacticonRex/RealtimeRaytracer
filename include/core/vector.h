...
#define FULLCTOR( n, a,b,c,d ) \
	...
	vec##n##_t(std::initializer_list<TEMPLATE_NAME> args) { \
		if ( args.end() - args.begin() != n ) \
			Error.send( "vec_" #n "_t.ctor(initializer_list)", \
						ERROR_TYPE::INIT, \
...
TEMP1 union vec2_t {
	...
	struct { TEMPLATE_NAME first,second; };
...
TEMP1 union vec3_t {
	...
	struct { TEMPLATE_NAME first,second,third; };
...
TEMP1 union vec4_t {
	...
	struct { TEMPLATE_NAME first,second,third,fourth; };
...
// vector3 exclusive functions
TEMP1 VECN(3) Cross( const VECN(3)& a, const VECN(3)& b ) {
	return VECN(3)( a.y*b.z - a.z*b.y,
					a.z*b.x - a.x*b.z,
					a.x*b.y - a.y*b.x );
}
TEMP1 VECN(3) Reflect( const VECN(3)& input, const VECN(3)& normal ) {
	float32 dot = Dot( input, normal );
	VECN(3) against = input - dot * normal;
	VECN(3) with = dot * normal;
	return against - with;
}
