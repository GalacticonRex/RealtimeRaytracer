...
eulerangles_t toEuler(const axisangle_t& n)
{
	...
	float32 val = n.axis.x()*n.axis.z()*t+n.axis.y()*s;
...
axisangle_t toAxisAngle(const eulerangles_t& n)
{
	...
	fvec v( c1*c2*s3 + s1*s2*c3,
			c1*s2*c3 - s1*c2*s3,
			s1*c2*c3 + c1*s2*s3 );
	
	if ( v.length() < 0.0000001 ) return axisangle_t();
	return axisangle_t( a, fvec::normalize( v ) );
}
...
axisangle_t toAxisAngle(const quaternion_t& q) {
	if ( q.w == 1 ) {
		return axisangle_t( fvec(1,0,0),0 );
	} else {
		float32 s = sqrt( 1 - q.w * q.w );
		return axisangle_t( fvec(q.i/s,q.j/s,q.k/s),2 * acos( q.w ) );
	}
}
...
quaternion_t toQuaternion(const eulerangles_t& n)
{
	...
	float32 i = c1*c2*c3 - s1*s2*s3, 
			   a = 2 * acos( i ),
			   s = sin( a/2. );
	fvec v = fvec::normalize( fvec( s1*s2*c3 + c1*c2*s3,
									c1*s2*c3 - s1*c2*s3,
									s1*c2*c3 + c1*s2*s3 ) ) * s;
